...
\includegraphics[width=0.49\textwidth]{plots/processes/all_expt/Allexpt_corrdr_xQ+1_f8_samept}
...
\includegraphics[width=0.49\textwidth]{plots/processes/all_expt/Allexpt_corr_hist+1_f0_samept}
...
\includegraphics[width=0.49\textwidth]{plots/processes/all_expt/Allexpt_corrdr_hist+1_f0_samept}
...
\begin{figure*}
 \ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/processes/jet/new/corrdr_xQ+1_f8_samept} 

\includegraphics[clip,width=0.49\textwidth]{plots/processes/ttbar/corrdr_xQ+1_f8_samept}
\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/processes/Zpt/corrdr_xQ+1_f8_samept} 
...
\begin{figure*}
\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/processes/W/new/corrdr_xQ+1_f13_samept} 
...
\section{Case Study: New Collider Pseudodata}

\subsection{Case Study: LHeC and EIC \label{sec:Case-Study:-LHeC}}

(results are in ./Temporary\_Results/pseudo\_data\_tmp/ of 

https://smu.box.com/s/sdd9qbw6wvee6n7u6441j2f2jil7s2t7)
...
\end{enumerate}
\begin{figure*}
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/LHeCEIC_1y/corrdr_xQ+1_f0_samept}
\ \ \
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/LHeCEIC_1y/corrdr_xQ+1_f2_samept} 
...
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/LHeCEIC_1y/corr_xQ+1_f3_samept}
\ \ \
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/LHeCEIC_1y/corr_xQ+1_f-1_samept} 
...
\begin{figure*}
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/LHeCEIC_1y/dr_xQ+1_samept}
...
channel in the $|S_{f}|(x_{i},\mu_{i})$ plots of Fig.~\ref{fig:LHeCEIC1y-sens},
\ref{fig:LHeCEIC1y-corr}, \ref{fig:LHeCEIC1y-dr}. }
\label{tab:LHeC} 
\end{table}

\subsection{Case Study: After@LHC}

(results are in ./Temporary\_Results/pseudo\_data\_tmp/AfterLHC of 

https://smu.box.com/s/sdd9qbw6wvee6n7u6441j2f2jil7s2t7)

The idea After@LHC fixed target experiments is using high boost of
p-A collission and high liminosity to explore large-x (0.3\textasciitilde{}1)
PDFs, enabling us to explore abundant PDF knowledge. Following are
some opportunities of After@LHC:
\begin{enumerate}
\item large-x gluon and heavy quarks
\item Boer-Mulders effect (orrelation between the parton $k_{T}$ and its
spin)
\item Sivers effect (correlation between the parton $k_{T}$ and the proton
spin)
\end{enumerate}
We analyze the sensitivities from $d\sigma^{DY}/dy_{\mu\mu}^{cm}$
psuedodata for pp collisions at $\sqrt{S}=115GeV$ with the $10fb^{-1}$
luminosity and CT14NLO theoretical predictions. Because the psuedodata
does not attach correlated errors or shifted data, I use $(T_{i}-D_{noshift})/\sigma_{uncor}$
as residuals. Formulas of $(x,\mu)$ are: $\mu=Q=M_{ll}$ $x_{1,2}=Q/\sqrt{S}e^{\pm y}$.

results:
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{Y}}{]} The mean $\delta r$ is 11.02 so
the sensitivities are in general very high. $x$ in (0.003, 0.1) of
$\bar{u}$ and $\bar{d}$ are the most sensitive (Fig. \ref{fig:AfterLHC-sens}).
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}{]} I still
don't know how long is the runtime of the data so I can not normalize
the sensitivity to 1 year runtime as the LHeC and EIC case.
\end{enumerate}
\begin{figure*}
\includegraphics[clip,width=0.6\textwidth]{plots/Pseudo_Data/AfterLHC/corrdr_xQ+1_f-1_samept}
\ \ \

\caption{Select sensitivity plots for the After@LHC pseudodata, in this case
showing $|S_{\bar{u}}|(x_{i},\mu_{i})$.}
\label{fig:AfterLHC-sens} 
\end{figure*}
...
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}{]} (https://data.nnpdf.science/talks/rojo-NNPDFs-KCL.pdf)
PDF uncertainties are important for following $\sigma_{H}(8TeV)$
channels: gg-\textgreater{}H, VBP, WH, ZH, ttH; Higgs decay channels:
$H->\tau\tau$, H-\textgreater{}ZZ, H-\textgreater{}WW, $H->\gamma\gamma$;
SUSY heavy productions: $\stackrel{\sim}{g}\stackrel{\sim}{g}X$,
$\stackrel{\sim}{q}\stackrel{\sim}{q}^{*}X$; large-x gluon (improve
the accuracy of high mass graviton)
...
flavour 
\begin{figure*}
\includegraphics[clip,width=0.6\textwidth]{plots/constrain_quantities/sens_correrror_+1_f8_thumbnail}
...
(results are in PDFsense\_Tasks\_and\_Results of SMU box: https://smu.box.com/s/dcuo0cjwfdsniuhmxwmn45njkmiy36jo)

We can do the Mellin transformation on PDFs to get Mellin moments
$\langle x{}^{N}\rangle_{F}=\int_{0}^{1}dx\ x^{N}f(x)$. With enough
orders of $N$, we can reconstruct PDFs $f(x)$. Nowadays, we are
capable of estimating the first few Mellin moments via Lattice QCD
calculations, so we can compare PDFs from fits and lattice QCD calculations.
One application is using lattice calculation to constrain PDFs in
the fitting processes (via giving $\chi^{2}$ penalties to PDFs that
are incompatible with lattice calculations). We calculate the sensitivities
of datasets in the HERA2 fit and some LHC datasets to the first three
Mellin moments for some flavours, which demonstrates datasets imposing
the most constraint to these Mellin moments, where the PDFset is CT14HERA2NNLO
and the factorization scale is 4 GeV. We also calculate $\langle(1-x)^{N}\rangle_{F}$
to identify datasets that are sensitive to small-x PDFs as the $x^{N}$
terms are only sensitive to larger x PDFs.

\textbf{({*} PN {*})} The $\langle(1-x)^{N}\rangle_{F}$ moments may
be sensitive to the small-$x$ behavior for the PDF combinations that
are integrable in the limit $x\rightarrow0$. A $\langle(1-x)^{N}\rangle_{F}$
...
{[}\textbf{\textcolor{blue}{Y}}{]} For $\langle x{}^{N}\rangle_{F}$,
we study sensitivities of g, u+, d+, s+, u- -d-, T3, T8, T24, s-d,
c-s, b-c mellin moments. And for $\langle(1-x)^{N}\rangle_{F}$, we
study sensitivities of u-,d-,u-+d-,u- -d-. We select PDFs of these
flavours for the effects of $(1-x)^{N}$ in the small-x region are
prominent due to their convergence in the small-x region. Where $N=0\sim3$.
Statistically, datasets that are most sensitive to the investigated
Mellin moments are ID = 101\textasciitilde{}111 (BCDMS, NMC,CDHSW,
CCFR), 124\textasciitilde{}127 (NuTeV, CCFR), 160 (HERA2), 201\textasciitilde{}203
(E605, E866), 542\textasciitilde{}545 (LHC jet datasets).

need to add:
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{2}}\textbf{\textcolor{red}{!!!}}{]}
N=-1 for Figures of $\langle(1-x)^{N}\rangle_{F}$: according to https://www.jlab.org/indico/event/252/session/17/contribution/113/material/slides/0.pdf,
Mellin moments necessary for reconstructing pion PDFs are N=-1\textasciitilde{}2. 
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{2}}\textbf{\textcolor{red}{!!!}}{]}
N=-1 for ranking tables of figures of $\langle x{}^{N}\rangle_{F}$
and $\langle(1-x)^{N}\rangle_{F}$ 
\end{enumerate}
Following are results of $\langle x{}^{N}\rangle_{F}$:
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{Y}}{]} According to Table. x, the most
sensitive flavours are correlated with the gluon PDF (g, B-C).
\item {[}\textbf{\textcolor{blue}{Y}}{]} when N become larger, Mellin moments
are sensitive to larger x PDFs. Besides, for $N>0$, the high sensitive
regions are in (0.01, 1), indicating that Mellin moments can not resolve
x\textless{}0.01 regions of PDFs for $N>0$. The results are due to
the log-scale insensitivity of $x^{N}$. We will also see in the $\langle(1-x)^{N}\rangle_{F}$
paragraph, even $(1-x)^{N}$ terms are not sensitive to $x<0.01$
regions.
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{3}}\textbf{\textcolor{red}{?}}{]}
According to 2, whether we need to figure out new Mellin moments to
resolve PDFs below $x=0.01$ 
\item {[}\textbf{\textcolor{blue}{Y}}{]} We find, for most N\textgreater{}0
Mellin moments, only narrow x bands of data points are strong correlated
with them ($C(\langle x{}^{N}\rangle_{a},r_{i})$\textgreater{}0.7)
(Fig. \ref{fig:Mellin_s} left panels). Moreover, $C(\langle x{}^{N}\rangle_{a},f_{a}(x,\mu))$
versus x also have the most strong correlation in these x bands (Fig.
\ref{fig:Mellin_s} right panels). The finding shows that Mellin moments
are mainly determined by small-x ranges of PDFs. Therefore, we can
substantially increase accuracies of Mellin moments by constraining
PDFs in these x ranges. 

{[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}\textbf{\textcolor{red}{!}}{]}
However, for some Mellin moments, we still can not explain why peaks
of $C(\langle x{}^{N}\rangle_{a},f_{i,a}(x_{i},\mu_{i}))$ and $C(\langle x{}^{N}\rangle_{a},r_{i})$
are not exactly at the same x bands or totally different, which means
we don't know whether constraining $f_{i,a}(x_{i},\mu_{i})$ are still
useful for constraining $\langle x{}^{N}\rangle_{a}$. For example,$\langle x{}^{0}\rangle_{s+}$
is correlated with data with x\textasciitilde{}0.1 but being correlated
with $s+(x,\mu)$ with x smaller than 0.01 (Fig. \ref{fig:Mellin_s}
left-up and right-up panels). For $\langle x{}^{0}\rangle_{u+}$,
the case is that it is only correlated with $u+(x,\mu)$ with x smaller
than $10^{-4}$ but being correlated with data with x\textless{}0.005
(Fig. \ref{fig:Mellin_s} left-down and right-down panels). If the
strong $C(\langle x{}^{N}\rangle_{a},r_{i})$ are due to the strong
$C(\langle x{}^{N}\rangle_{a},f_{i,a}(x_{i},\mu_{i}))$, it is reasonable
to believe that $C(\langle x{}^{N}\rangle_{a},f_{i,a}(x_{i},\mu_{i}))>C(\langle x{}^{N}\rangle_{a},r_{i})$,
which contradicts to $\langle x{}^{0}\rangle_{u+}$ case.
\item {[}\textbf{\textcolor{blue}{Y}}{]} $(u^{-}-d^{-})x^{0}$ should be
1 (constant) but its total sensitivity is the same amount compared
with most Mellin moments (see row 1 and 2 in \ref{tab:Mellin_total_sens}).
We can lower the order of precision to reduce the fluctuations on
it. However, it also indicates that even physical quantities have
no correlation/sensitivity with datasets/PDFs, the numerical fluctuations
still contribute some sensitivities to PDFs. Therefore, to avoid considering
the sensivities from fluctuations, perhaps we don't want to consider
too low sensitivity as the real sensitivity. Perhaps we also want
to figure out ways to remove the fluctuations or identify whether
the sources of sensitivities are fluctuations.

Solution: because the $(u^{-}-d^{-})x^{0}$ uncertainty ratio is very
small (under $10^{-4}$), we do not need to discuss its' sensitivity.
Usually uncertainties of physical qunatities are larger than 0.1\%,
so signals of numerical errors would not impact the sensitivities
or correlations.
\end{enumerate}
Following are results of $\langle(1-x)^{N}\rangle_{F}$:
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{Y}}{]} We also make ranking tables for
$\langle(1-x)^{N}\rangle_{F}$
\item {[}\textbf{\textcolor{blue}{Y}}{]} As mentioned before, $C(\langle(1-x){}^{N}\rangle_{f},r_{i})$
are strong only in x\textgreater{}0.01 (Fig. \ref{fig:Mellin_u-1-x})
except for T24 (percept T24 is associated with the gluon PDF). Hence,
$\langle(1-x)^{N}\rangle_{F}$ can not resolve small-x PDFs.
\end{enumerate}

\textcolor{red}{note: the total sensitivity table (Tab. \ref{tab:Mellin_total_sens})
uses the old formula, it should be modify latter}

\begin{figure*}
\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_data_x/corr_xQ+1_f22_samept}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_PDF_corr_x/splus_Melling_PDF_CT14HERA2NNLO_Q4} 

\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_data_x/corr_xQ+1_f25_samept}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_PDF_corr_x/uplus_Melling_PDF_CT14HERA2NNLO_Q4} 

\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_data_x/corr_xQ+1_f10_samept}
\ \ \

\caption{Left: Correlation of the HERA2+LHC datasets and the 0st and 3rd order
s+ and u+ Melling moments. Right: Correlation of s+, u+ PDFs and their
respective Mellin moments. The factorization scales of Mellin moments
and PDFs are 4 GeV. }
\label{fig:Mellin_s} 
\end{figure*}

\begin{figure*}
\includegraphics[clip,width=0.49\textwidth]{plots/MellinMoment/mellin_moment_data_1-x/corr_xQ+1_f9_samept}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/MellinMoment/mellin_moment_PDF_corr_1-x/uminus_Melling_PDF_CT14HERA2NNLO_Q4} 

\caption{Left: Correlation of the HERA2+LHC datasets and the 0st and 3rd order
u- (1-x) Melling moments. Right: Correlation of u- PDFs and its' respective
(1-x) Mellin moments. The factorization scales of Mellin moments and
PDFs are 4 GeV. }
\label{fig:Mellin_u-1-x} 
\end{figure*}
...
\begin{figure*}
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2/residual_xQ+1_samept_thumb}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2NoJet/residual_xQ+1_samept_thumb} 

\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2/dr_xQ+1_samept_thumb}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2NoJet/dr_xQ+1_samept_thumb} 
...
\begin{figure*}
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2/corr_xQ+1_f0_samept_thumb}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2NoJet/corr_xQ+1_f0_samept_thumb} 

\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2/corrdr_xQ+1_f0_samept_thumb}
\ \ \
\includegraphics[clip,width=0.45\textwidth]{plots/HERA2_HERA2NoJet/HERA2NoJet/corrdr_xQ+1_f0_samept_thumb} 
...
\end{enumerate}
\begin{figure*}
\includegraphics[clip,width=0.49\textwidth]{plots/HERA2_HERA2NoJet/tensorprojector/pca}
\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/HERA2_HERA2NoJet/tensorprojector/tsne}
...
\begin{figure*}
\includegraphics[clip,width=0.49\textwidth]{plots/sens_xrange/Sens_xrange_HERA2plusLHC_CT14HERA2NNLO_Q1_3}
\ \ \
\includegraphics[width=0.49\textwidth]{plots/sens_xrange/Sens_xrange_HERA2plusLHC_CT14HERA2NNLO_Q125}\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/sens_xrange/HERA2_LHC_JP_TTbar_Zpt_singlesens_g} 
...
\end{enumerate}
\begin{figure*}
\includegraphics[clip,width=0.49\textwidth]{plots/LinearFitPDFs/Corr_dfxQoverdTPearsonC_247}
\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/LinearFitPDFs/BestFit_coefsdfNorm_vs_x_203}
\ \ \
\includegraphics[clip,width=0.49\textwidth]{plots/LinearFitPDFs/BestFit_coefsRatio_vs_x_203}
