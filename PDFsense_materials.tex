...
DIS NC and CC datasets \ref{tab:LHeC}. \textcolor{red}{note: we will
need to further analyze the sensitivity of LHeC and EIC, so we should
update the result with new .dta files} 
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{Y}}{]} LHeC DISNC datasets constrain the
small x region of the gluon PDF \ref{fig:LHeC-sens}, \ref{fig:LHeC-corr}
\item {[}\textbf{\textcolor{blue}{Y}}{]} LHeC DISCC and DISNC datasets constrain
u and d at large x and small x regions. For the small x region, the
high sensitivity results from the fact that few constraints in that
region 
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}\textbf{\textcolor{red}{!}}{]}
LHeC DISCC data is correlated to middle x region (10\textasciicircum{}-3\textasciitilde{}10\textasciicircum{}-2)
s PDF, I don't know why \ref{fig:LHeC-corr}. Perhaps the strong correlation
is due to the large uncertainty of the strange PDF, which means this
region some DISCC data could constrain strange at this region. 
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}\textbf{\textcolor{red}{!}}{]}
I don't know why the diagonal part of 192 (CC), 194 (NC) points in
Fig. \ref{fig:LHeC-corr} are strongly correlated with the ubar PDF 
\item {[}\textbf{\textcolor{blue}{Y}}{]} the large $\delta r$ enhance the
sensitivities of LHeC data to PDFs especially at the small x region
\ref{fig:LHeC-dr}
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{1}}\textbf{\textcolor{red}{!!!}}{]}
we also want to draw the sensitivity plots for the EIC at various
energies and luminosities, using the same setting as for the LHeCs.
With the .dta files, we can generate them instantly. 
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{1}}\textbf{\textcolor{red}{!!!}}{]}
Make the ranking tables for 1-year and 10-years runtime of LHeC and
JLabEIC. (update: plots for 1y and 10y: Both LHeC and EIC NC datasets
are sensitive to u \ref{fig:LHeCEIC_1y}. Ranking tables are not finished
yet)
\end{enumerate}
\begin{figure*}
\includegraphics[clip,width=0.3\textwidth]{plots/LHeC/corrdr_xQ+1_f0_samept}
\ \ \
\includegraphics[clip,width=0.3\textwidth]{plots/LHeC/corrdr_xQ+1_f2_samept} 

\caption{Select sensitivity plots for the LHeC pseudodata, in this case showing
$|S_{g}|(x_{i},\mu_{i})$ in the left panel, $|S_{d}|(x_{i},\mu_{i})$
...
\includegraphics[clip,width=0.3\textwidth]{plots/LHeC/corr_xQ+1_f3_samept}
\ \ \
\includegraphics[clip,width=0.3\textwidth]{plots/LHeC/corr_xQ+1_f-1_samept} 

\caption{Select correlation plots for the LHeC pseudodata, in this case showing
$|C_{s}|(x_{i},\mu_{i})$ in the left, and $|C_{\overline{u}}|(x_{i},\mu_{i})$
in the right. The pseudodata are broken among four separate reaction
channels \textemdash{} CC $e^{-}p$ (circles) and $e^{+}p$ (squares),
as well as NC $e^{-}p$ (diamonds) and $e^{+}p$ (triangles). }
\label{fig:LHeC-corr} 
...
\begin{figure*}
\includegraphics[clip,width=0.3\textwidth]{plots/LHeC/dr_xQ+1_samept}

\caption{Select $\delta r$ plots for the LHeC pseudodata, in this case showing
$\delta r_{i}$ in the left panel. The pseudodata are broken among
four separate reaction channels \textemdash{} CC $e^{-}p$ (circles)
and $e^{+}p$ (squares), as well as NC $e^{-}p$ (diamonds) and $e^{+}p$
(triangles). }
\label{fig:LHeC-dr} 
\end{figure*}

\begin{figure*}
\includegraphics[clip,width=0.3\textwidth]{plots/EIC/corrdr_xQ+1_f1_samept_1y}

\caption{The sensitivity plot for 1 year pseudodata of LHeC and EIC datasets.}
\label{fig:LHeCEIC_1y} 
\end{figure*}
...
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}{]} sensitivities
of correlated systematic errors to various physical observables (Higgs
cross sections, weak angle, etc.)

I calculate the PDF sensitivities $S_{f}(\sigma_{corr,i},f_{a}(x,\mu))$
and sensitivities of physical quantities $S_{f}(\sigma_{corr},Q_{i})$
imposed by the experimental correlated errors $i$. For the PDF sensitivities,
I explore the sensitivities in x interval $[10^{-5},0.9]$ and flavours
$a$ at $\mu=5GeV$ and find the maximum sensitivity $|S_{f}(\sigma_{corr,i},f_{a}(x_{max},\mu=5GeV))|$
in the x range. I draw contour plots (Fig. \ref{fig:Sens_corr_error})
to indicate the sensitivities of $\beta_{ID,j}\equiv\sigma_{corr,i}$
with x, y axes representing $(ID,j)$. Where ID indicates the experimental
ID and j indicates the j-th correlated error of the corresponding
ID in .dta files (labels are like \char`\"{}r(k) = $R^{2}$ b1 b2
b3......\char`\"{}, I exclude $R^{2}$ here).

I also make the $\beta_{ID,j}$ list with high sensitivities to PDFs
and quantities (in MaxSenscorrerror\_info.txt). I select only $|S_{f}(\sigma_{corr,i},f_{a}(x_{max},\mu=5GeV))|>1$
for the flavour $a$ and $\beta_{ID,j}$. I find that most $\beta_{ID,j}$
with large sensitivities are in ID = 247, 249, 542, 544, 545, 566,
568. Especially, $\beta_{ID,j}$ of 249 are the most sensitive among
all correlated errors; Almost all $|S|>3$ $\beta_{ID,j}$ are in
249.
\begin{figure*}
\includegraphics[clip,width=0.3\textwidth]{plots/constrain_quantities/sens_correrror_+1_f8}

\caption{The sensitivity plot for }
...
We want to know how the data constrain the PDF space rather than just
know the sensitivity to each flavour. Advantage: We don't need to
guess and test which combination of flavour has the strongest constraint
by the user function again and again.

Other advantages: if we know the relation between the theoretical
values and PDFs, whether we can directly estimate the scale of PDF
uncertainties imposed by experimental measurements or at least get
more insight understanding of how measurements constrain PDFs? 

We can approximate a linear relation among theoretical prediction
and PDFs at the specified $(x,\mu)$. Here we temporary ignore the
nonlinear terms, the correlation of replicas among various flavours,
and the contribution from PDFs that are not at $(x,\mu)$ and we absorb
the unknown factors into a variable $X$. Then with
...
\[
|\delta T_{i}|^{2}=\underset{R}{\sum}\delta T_{i,R}^{2},|\delta f_{i}|^{2}=\underset{R}{\sum}\delta f_{i,R}^{2}
\]

We have the following rules:

\[
T_{i,r}=\underset{a}{\sum}c_{i,a}f(x_{i},\mu_{i})_{a,r}+X_{i,r}
\]
...
the Hessian uncertainty rule: $\delta Q=\sqrt{\underset{R}{\sum}\delta Q_{R}^{2}}$
and the squre of the addition of two vectors is $(\stackrel{\rightharpoonup}{Q}_{1}+\stackrel{\rightharpoonup}{Q}_{2})^{2}=Q_{1}^{2}+Q_{2}^{2}+2|Q_{1}||Q_{2}|\rho$
for the correlation $\rho$, the total theoretical uncertainty for
each point $i$ is:

\[
|\delta T_{i}|^{2}=\underset{R}{\sum}(\underset{a}{\sum}c_{i,a}\delta f_{i,a,R}+\delta X_{i,R})^{2}\equiv(\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a})^{2}+(\delta(\delta X_{i}))^{2}+2|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}||\delta(\delta X_{i})|\rho\simeq(\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a})^{2}+(\delta(\delta X_{i}))^{2}
\]

Where we assume the unknown quantity $X$ and the known PDF combination
is uncorrelated, which means that the vectors $\stackrel{\rightharpoonup}{\delta X_{i}}$,
...
Therefore, the Hessian uncertainty correlation between $T_{i}$and
$\underset{a}{\sum}c_{i,a}f_{i,a}$ is $C(T_{i},\underset{a}{\sum}c_{i,a}f_{i,a})\simeq\frac{|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}|}{|\delta T_{i}|}$.
If even the various PDFs of flavours are uncorrelated with each other,
we have $C(T_{i},f_{i,a})\simeq\frac{\delta(c_{i,a}|\delta f_{i,a}|)}{|\delta T_{i}|}$.
...
of quantities $f_{\bar{d}}$, $f_{\bar{u}}$, $f_{g}$, $f_{u}$,
$f_{d}$, $f_{s}$, and unknown $X$. Therefore, $c_{\bar{d}}(x,\mu),c_{\bar{u}}(x,\mu),c_{g}(x,\mu),c_{u}(x,\mu),c_{d}(x,\mu),c_{s}(x,\mu)$
specify the relation between uncertainties on PDFs of flavours and
experimental measurements. $C(T_{i},\underset{a}{\sum}c_{i,a}f_{i,a})$
is the goodness of the specification. 

\subsubsection{Setup}

for each dataset, using the linear fit $T_{i,r}=\underset{a}{\sum}c_{i,a}f(x_{i},\mu_{i})_{a,r}+const$
to fit the 56 replicas for each point $i$
...
ideas.
\begin{enumerate}
\item {[}\textbf{\textcolor{blue}{Y}}{]} by checking the relation $C(T_{i},\underset{a}{\sum}c_{i,a}f_{i,a})\simeq\frac{|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}|}{|\delta T_{i}|}$
...
sensitivity $S(T_{i},\underset{a}{\sum}c_{i,a}f_{i,a})\equiv\frac{\delta r_{i}}{r_{ID,rms}}C(\delta r_{i},\underset{a}{\sum}c_{i,a}f_{i,a})\simeq\frac{|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}|}{\sigma_{uncor,i}r_{ID,rms}}\simeq\frac{|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}|}{\sigma_{uncor,i}}$,
...
In other words, in which conditions $\frac{|\delta(\delta f_{i,a})|}{\sigma_{uncor}}=\delta r_{i}C(r_{i},f_{i,a})$
\item {[}\textbf{\textcolor{blue}{N}}\textbf{\textcolor{teal}{?}}\textbf{\textcolor{red}{?}}{]}
...
$C(T_{i},\underset{a}{\sum}c_{i,a}f_{i,a})\simeq\frac{|\delta\underset{a}{\sum}c_{i,a}\delta f_{i,a}|}{|\delta T_{i}|}$
...
uncertainties. I don't know whether the instability will disappear
when considering the $\mu$ dependency.
